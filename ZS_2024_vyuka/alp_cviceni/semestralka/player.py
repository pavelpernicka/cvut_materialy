    ...
    def game_step(self):
        ...
        def connected_components_size(matrix, start_row, start_col, target_value, visited, placed_mask, debug_path=None):
            ...
            while queue:
                ...
                if not placed_mask[r, c]:
                    outreaching = True

                # Add neighbors
                queue.extend([(r - 1, c), (r + 1, c), (r, c - 1), (r, c + 1)])

            return size if outreaching else 0

        best_card, best_score, best_position, best_rotation = None, -1, None, 0
        visited = set()
        ...
        for card in self.available_cards:
            for rotation in range(4):
                card.set_best_rotation(rotation)
                card_rows, card_cols = card.size

                for row in range(self.rows - card_rows + 1):
                    for col in range(self.columns - card_cols + 1):
                        # Skip invalid placements early
                        if not self.check_touch(row, col, card_rows, card_cols):
                            continue

                        card.position = (row, col)
                        temp_matrix = self.game_matrix.copy()
                        temp_matrix[row:row + card_rows, col:col + card_cols][card.matrix >= 0] = card.matrix[card.matrix >= 0]

                        placed_mask = np.zeros_like(temp_matrix, dtype=bool)
                        placed_mask[row:row + card_rows, col:col + card_cols][card.matrix >= 0] = True

                        total_score = sum(
                            connected_components_size(temp_matrix, r, c, card.matrix[r, c], visited, placed_mask)
                            for r in range(card_rows) for c in range(card_cols) if card.matrix[r, c] > 0
                        )

                        if total_score > best_score:
                            best_card, best_score = card, total_score
                            best_position, best_rotation = (row, col), rotation

        if best_card:
            best_card.position = best_position
            best_card.set_best_rotation(best_rotation)
